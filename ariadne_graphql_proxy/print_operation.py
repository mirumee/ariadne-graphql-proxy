...
from .exceptions import InvalidOperationNameError
...
def print_selection_set(selection_set: SelectionSetNode, *, indent: int = 2) -> str:
    ...
    for selection in selection_set.selections:
        if isinstance(selection, FieldNode):
            selections.append(print_operation_field(selection, indent=indent))
...
def print_operation_field(field_node: FieldNode, *, indent: int = 2) -> str:
    ...
    if field_node.arguments:
        field_str += f"({print_operation_field_arguments(field_node)})"
...
def print_operation_field_arguments(field_node: FieldNode):
    arguments = []

    for argument in field_node.arguments:
        name = argument.name.value
        value = print_value_node(argument.value)
        arguments.append(f"{name}: {value}")

    return ", ".join(arguments)


def print_value_node(value_node: ValueNode) -> str:
    ...
    if isinstance(value_node, (ListValueNode, ConstListValueNode)):
        items = ", ".join(
            print_value_node(item_value) for item_value in value_node.values
        )
        return f"[{items}]"
    ...
    if isinstance(value_node, (ObjectValueNode, ConstObjectValueNode)):
        ...
        for field in value_node.fields:
            ...
            field_value = print_value_node(field.value)
